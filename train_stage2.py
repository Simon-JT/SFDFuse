...
from net import UnifiedFusionNetwork, FixedKernelLFSM, DWTLFSM
...
param_grid = {
    ...
    # DWT parameters
    'wavelet': ['db1'],  # 可选: 'db1', 'haar', 'sym2', 'coif1'
    'level': [1]         # 可选: 1, 2, 3
}
...
def train_with_params(params):
    print("="*50)
    print("Stage 2: Training Fusion Network (DWT LFSM)")
    print("="*50)
    ...
    # Initialize models with DWT frequency separator
    unified_network = nn.DataParallel(
        UnifiedFusionNetwork(
            freq_separator_type='dwt',
    ...
    try:
        ...
        for epoch in range(num_epochs):
            ...
            for i, (vis_img, ir_img, mask, _) in enumerate(trainloader):
                ...
                # 获取分离的高低频分量
                freq_components = unified_network.module.freq_separator(vis_img, ir_img)
            ...
            if valid_batches > 0:
                ...
                if avg_loss < best_loss:
                    ...
                    model_path = os.path.join("models", f"stage2_best_dwt_{params['wavelet']}_{params['level']}_{timestamp}.pth")
...
def grid_search():
    ...
    results_file = f"stage2_grid_search_results_dwt_{datetime.datetime.now().strftime('%m-%d-%H-%M')}.json"
    all_results = []
    ...
    for i, params in enumerate(param_combinations):
        ...
        try:
            ...
            if current_loss < best_loss:
                ...
                # Save best model
                timestamp = datetime.datetime.now().strftime('%m-%d-%H-%M')
                best_model_path = os.path.join("models", f"stage2_best_dwt_{params['wavelet']}_{params['level']}_{timestamp}.pth")
...
if __name__ == "__main__":
    try:
        ...
        if best_state is not None:
            timestamp = datetime.datetime.now().strftime('%m-%d-%H-%M')
            final_path = os.path.join("models", f"final_stage2_dwt_{best_params['wavelet']}_{best_params['level']}_{timestamp}.pth")
            torch.save(best_state, final_path)
            print(f"\nFinal model saved to: {final_path}")
            
            # Save best parameter configuration
            config_path = os.path.join("models", f"stage2_best_config_dwt_{timestamp}.json")
        ...
        if 'best_state' in locals() and best_state is not None:
            emergency_path = os.path.join("models", f"emergency_stage2_dwt_{datetime.datetime.now().strftime('%m-%d-%H-%M')}.pth")
